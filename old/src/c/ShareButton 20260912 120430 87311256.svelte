<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      if (!p.endsWith('/')) p += '/';
      const res = await fetch(siteConfig.analytics.pageViewsApi + '?p=' + encodeURIComponent(p), { signal: AbortSignal.timeout(5000) });
  ...
  function shareToQQ() {
    window.open('https://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=' + encodeURIComponent(shareUrl) + '&title=' + encodeURIComponent(title) + '&desc=' + encodeURIComponent(description) + '&summary=' + encodeURIComponent(description) + '&site=' + encodeURIComponent(siteConfig.title), '_blank', 'width=700,height=600');
  }
  ...
  async function generatePoster() {
    ...
    try {
      ...
      ctx.textAlign = 'left';
      ctx.fillText(siteConfig.title, 40, 68);
      ...
      if (image) {
        ...
        ctx.fillRect(pad, y, contentW, 115);
        ctx.strokeStyle = '#0284c7';
        ctx.lineWidth = 4;
        ctx.strokeRect(pad, y, contentW, 115);
        ctx.fillStyle = '#0284c7';
        ctx.font = 'bold 42px "Fredoka", system-ui, sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText(siteConfig.title, pad + contentW / 2, y + 72);
        ctx.textAlign = 'left';
        y += 148;
      }
      ...
      ctx.clip();
      try {
        const avImg = await loadImage(siteConfig.url + siteConfig.assets.posterAvatar);
      ...
      ctx.textAlign = 'left';
      ctx.fillText(siteConfig.title, avX + av + 20, footerY + 58);
      ctx.fillStyle = '#0284c7';
  ...
  function downloadPoster() {
    if (posterDataUrl) {
      ...
      a.download = siteConfig.title.toLowerCase().replace(/\s+/g, '-') + '-poster-' + Date.now() + '.png';
      a.click();
    }
  }
</script>
...
<div use:portal class="fixed inset-0 z-[99999]" style="background: rgba(0,0,0,0.6); backdrop-filter: blur(4px);" role="dialog" aria-modal="true" on:click={closePoster}>
